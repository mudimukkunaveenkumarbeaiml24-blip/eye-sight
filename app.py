...
║         File  : app.py  (Vercel-ready, single-file)          ║
╚══════════════════════════════════════════════════════════════╝
...
DATABASE_PATH      → SQLite path (default: /tmp/neurovision.db  ← IMPORTANT for Vercel)
...
# ★ KEY FIX: Vercel filesystem is read-only except /tmp
# Use /tmp for SQLite on Vercel; fallback to local for dev
...
# ★ KEY FIX: uploads also go to /tmp on Vercel
UPLOAD_FOLDER   = Path(os.environ.get("UPLOAD_FOLDER", "/tmp/nv_uploads"))
UPLOAD_FOLDER.mkdir(parents=True, exist_ok=True)

# ★ KEY FIX: frontend folder — look next to this file first, then one level up
_HERE = Path(__file__).resolve().parent
FRONTEND_FOLDER = None
for _candidate in [
    _HERE / "frontend",           # /app/frontend  (Vercel default layout)
    _HERE.parent / "frontend",    # one level up   (monorepo layout)
    _HERE,                        # same folder    (flat layout)
]:
    if (_candidate / "index.html").exists():
        FRONTEND_FOLDER = _candidate
        break
if FRONTEND_FOLDER is None:
    FRONTEND_FOLDER = _HERE / "frontend"   # will 404 gracefully if missing
...
def get_db():
    if "db" not in g:
        ...
        # Auto-init schema on first connection
...
# ──────────────────────────────────────────────────────────────
#  WINDOWS / macOS / LINUX DISPLAY SETTINGS HELPERS
#  (unchanged from original — OS-level apply is best-effort)
# ──────────────────────────────────────────────────────────────
def apply_windows_settings(payload: dict) -> dict:
    ...
    cmd = [
        ...
        dark_mode_flag, auto_sunset_flag,
...
def apply_macos_settings(payload: dict) -> dict:
    ...
    if bright_cli:
        ...
        results.append({"setting": "brightness", "ok": False, "reason": "brightness CLI not installed"})
    return {"applied": all(r.get("ok", False) for r in results), "results": results, "os": "macos"}
...
def apply_linux_settings(payload: dict) -> dict:
        ...
        try:
            ...
            for line in disp_result.stdout.splitlines():
                if " connected" in line:
                    ...
                    break
            r = subprocess.run([xrandr, "--output", primary, "--brightness", str(bright_val)],
    ...
    if gsettings:
        ...
        try:
            r = subprocess.run([gsettings, "set", "org.gnome.desktop.interface", "color-scheme", theme],
                                capture_output=True, text=True, timeout=10)
            results.append({"setting": "dark_mode", "ok": r.returncode == 0})
        except Exception as e:
            results.append({"setting": "dark_mode", "ok": False, "error": str(e)})
    else:
        results.append({"setting": "dark_mode", "ok": False, "reason": "gsettings not found"})
    return {"applied": all(r.get("ok", False) for r in results), "results": results, "os": "linux"}
...
def recommend_profile_from_score(overall_score: int, best_acuity: float) -> str:
    ...
    elif overall_score >= 65 and best_acuity >= 0.5: return "mild"
    elif overall_score >= 40 and best_acuity >= 0.25: return "moderate"
    return "high"
...
# ──────────────────────────────────────────────────────────────
#  ROUTES — ROOT  ★ VERCEL FIX ★
# ──────────────────────────────────────────────────────────────

@app.route("/")
def index():
    """
    Serve index.html if it exists in the frontend folder,
    otherwise return a JSON health-check so the API is still reachable.
    """
    index_file = FRONTEND_FOLDER / "index.html"
    if index_file.exists():
        return send_from_directory(str(FRONTEND_FOLDER), "index.html")
    # Friendly fallback — confirms backend is alive even without frontend
    return jsonify({
        "status":    "NeuroVision AI backend is running ✓",
        "version":   "2.0.0",
        "api_docs":  "/api/health",
        "message":   "Place your index.html inside a /frontend folder to serve the UI here.",
    })
...
def save_settings(sid: str):
    ...
    db.execute("""
        ...
        ON CONFLICT(session_id) DO UPDATE SET
            brightness=excluded.brightness, font_size=excluded.font_size,
            blue_filter=excluded.blue_filter, display_zoom=excluded.display_zoom,
            dark_mode=excluded.dark_mode, contrast=excluded.contrast,
            auto_sunset=excluded.auto_sunset, reduce_white=excluded.reduce_white,
            text_spacing=excluded.text_spacing, vision_profile=excluded.vision_profile,
...
def user_stats():
    ...
    sessions = db.execute("""
        ...
        FROM test_sessions WHERE user_id=? ORDER BY created_at ASC LIMIT 20
    """, (uid,)).fetchall()
...
def health():
    return ok({
        ...
        "frontend_exists":  (FRONTEND_FOLDER / "index.html").exists(),
        "timestamp":        datetime.utcnow().isoformat() + "Z",
    })

# ──────────────────────────────────────────────────────────────
#  STATIC / FRONTEND  ★ VERCEL FIX ★
#  This catch-all MUST come LAST — after all /api/* routes
# ──────────────────────────────────────────────────────────────

@app.route("/<path:path>")
def serve_static(path):
    """Serve any file from the frontend folder if it exists."""
    target = FRONTEND_FOLDER / path
...
╠══════════════════════════════════════════════════════════════╣
║  DB      : {DATABASE_PATH:<50}║
║  Frontend: {str(FRONTEND_FOLDER):<50}║
╚══════════════════════════════════════════════════════════════╝
    """)
